...
import com.spring.dev.service.EmployeeServiceImpl;

/*

If you revisit EmployeeServiceImpl class in Previous post, you will see that EmployeeServiceImpl basically 
depends on EmployeeDao & Employee to fulfill all of it’s duties. Each of the EmployeeServiceImplmethod is 
using only of these objects to do it’s real job.

As explained above, in order to test EmployeeServiceImpl, we would need to provide these dependencies. In 
our example, we do it using Mockito framework. We provide mock of EmployeeDao by applying @Mock annotation 
on it. We also provide spy objects of Employee by applying @Spy annotations on them.
...
So, Make sure to call MockitoAnnotations.initMocks when using Mockito annotations, else those mocks will be 
useless for your tests.
...
public class EmployeeServiceImplTest
{
	...
	public void isEmployeeSsnUnique()
	{
		...
		when(dao.findEmployeeBySsn(anyString())).thenReturn(emp);
		Assert.assertEquals(
				employeeService.isEmployeeSsnUnique(emp.getId(), emp.getSsn()),
				true);
	}
