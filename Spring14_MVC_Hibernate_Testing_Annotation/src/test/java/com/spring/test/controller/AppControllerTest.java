...
import com.spring.dev.controller.AppController;
import com.spring.dev.model.Employee;
import com.spring.dev.service.EmployeeService;

/*

If you revisit AppController class in Previous post, you will see that AppController basically depends on 
EmployeeService , MessageSource, Employee, ModelMap & BindingResult to fullfill all of it’s duties. 
Each of the AppController method is using only of these objects to do it’s real job.

So in order to test AppController, we would need to provide these dependencies. In our example, we do it 
using Mockito framework. We provide mock of EmployeeService & MessageSource by applying @Mock annotation 
on them. We also provide spy objects of ModelMap , BindingResult & Employee by applying @Spy annotations 
on them.

It’s important to understand that Mockito’s @Mock objects are not real instances, they are just bare-bones 
of instance created using Class of type. But their main capability is that they can remember all the 
interactions [operations performed] on them.

@Spy objects are on the other hand real instances, but with additional capabilities of remembering all the 
interactions [operations performed] on them.

@InjectMocks creates an instance of the class and injects the mocks that are created with the @Mock/@Spy 
objects in it.

MockitoAnnotations.initMocks(this); initializes objects annotated with Mockito annotations [@Mock, @Spy, @Captor, @InjectMocks]

Make sure to call MockitoAnnotations.initMocks when using Mockito annotations, else those mocks will be useless 
for your tests.
...
when..then & verify are popular stubbing and verification techniques used in tests to define the behavior and 
then optionally verifying that behavior was indeed executed.

There are many more. Please refer to TestNG tutorial for in depth details about writing tests using TestNG, 
mockito and other supportive libraries.
...
public class AppControllerTest
{
	...
	public void saveEmployeeWithValidationError()
	{
		...
		Assert.assertEquals(
				appController.saveEmployee(employees.get(0), result, model),
				"registration");
	}
	...
	public void saveEmployeeWithValidationErrorNonUniqueSSN()
	{
		...
		when(service.isEmployeeSsnUnique(anyInt(), anyString())).thenReturn(
				false);
		Assert.assertEquals(
				appController.saveEmployee(employees.get(0), result, model),
				"registration");
